...
/**
 * Swap given parameters into a dictionary of default parameters. Given 
 * parameters can be null or false and will still be swapped. Given parameters 
 * not included in the default set will be added, but I'm not sure why you'd 
 * want to do that.
 * @param {Object} def A dictionary of default parameters.
 * @param {Object} given A dictionary of given parameters to substitue for 
 * defaults.
 * @return {Object} The default dictionary with values from the given 
 * dictionary.
 */
...
/**
 * Combine a list of images into a single multi-band image. This is a 
 * convenience function over repeatedly calling addBands for each image you 
 * want to combine.
...
/**
 * Perform band-wise normalization on an image to convert values to range 0 - 
 * 1.
...
/**
 * Perform band-wise cyclic encoding with a sine or cosine transformation and 
 * rescale to range 0 - 1.
 * @param {ee.Image} img An image with cyclic data.
 * @param {object} [optionalParameters] A dictionary of optional parameters to 
 * override defaults.
 * @param {string} [optionalParameters.transform] The encoding transformation 
 * to apply, either "sin" or "cos". Default transformation is "sin".
 * @param {number} [optionalParameters.scale] The scale, in image units, to 
 * calculate image statistics at.
 * @param {ee.Geometry} [optionalParameters.region] The area to calculate image
 * statistics over.
 * @param {number, default 1e13} [optionalParameters.maxPixels] The maximum 
 * number of pixels to sample when calculating image statistics.
 * @return {ee.Image} The input image with all bands sine or cosine transformed
 * and rescaled between 0 and 1.
 */
...
/**
 * Perform band-wise stretching of an input image using a list of stretch 
 * values. For example, stretching an RGB image with a stretch list [1, 2, 1] 
 * would double values of the green channel
 * @param {ee.Image} img A multiband image to stretch.
 * @param {list} stretch A list of stretch factors to apply band-wise to the 
 * image. The number of stretch values must be equal to the number of image 
 * bands.
